...
fn mk_t(cx: ctxt, st: sty) -> t {
    ...
    match &st {
      ...
      &ty_trait(_, ref substs, _, _, _) => {
          flags |= sflags(substs);
          match st {
              ty_trait(_, _, RegionTraitStore(r), _, _) => { flags |= rflags(r); }
              _ => {}
          }
      }
...
pub fn fold_regions_and_ty(
    ...
    match tb.sty {
      ...
      ty_trait(def_id, ref substs, store, mutbl, bounds) => {
                 ...
                 adjustment: Option<@AutoAdjustment>) -> ty::t
{
    ...
    fn borrow_obj(cx: ctxt, span: span, r: Region,
        ...
        match get(ty).sty {
            ty_trait(trt_did, ref trt_substs, _, _, bounds) => {
