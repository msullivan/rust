...
impl<'self> LookupContext<'self> {
    ...
    pub fn search_for_autosliced_method(&self,
        ...
        match sty {
            ...
            ty_trait(trt_did, trt_substs, _, _, bounds) => {
                ...
                self.search_for_some_kind_of_autorefd_method(
                    ...
                    |trt_mut, reg| {
                        ty::mk_trait(tcx, trt_did, trt_substs.clone(),
                                     RegionTraitStore(reg), trt_mut, bounds)
                    })
            }
                                                ...
                                                candidate: &Candidate) -> ty::t
    {
        ...
        match candidate.method_ty.explicit_self {
            ...
            ast::sty_region(*) | ast::sty_box(*) | ast::sty_uniq(*) => {
                ...
                // XXX: probably bogus -sully
                let builtin_bounds = ty::EmptyBuiltinBounds();
        ...
        return match candidate.method_ty.explicit_self {
            ...
            sty_uniq => {
                ...
                match ty::get(rcvr_ty).sty {
                    ...
                    ty::ty_trait(self_did, _, UniqTraitStore, self_m, _) => {
                        mutability_matches(self_m, ast::m_imm) &&
                        rcvr_matches_object(self_did, candidate)
                    }
